...
{
private:
    std::vector< std::vector< double > > dane;             // Głowne dane macierzy, w postaci wektora 2D (macierzy)
...
public:
    ...
    Matrix(const std::vector< std::vector< double > >& wartosci) : wiersze(wartosci.size()), dane(wartosci)
    {
        ...
        for (const auto& wiersz : wartosci)
        {
            ...
            {
                throw std::invalid_argument("Wszystkie wiersze muszą miec dokladnie tyle samo kolumn");
            }
        }
    }
    ...
    // Obliczanie wyznacznika przy użyciu biblioteki Eigen
